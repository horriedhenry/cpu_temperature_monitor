...
typedef struct Time {
    ...
    std::string sc;
    std::string am_pm;
    std::string time() const { return hr + ":" + mn + ":" + sc + am_pm; }
} Time;
...
class CpuTemps {
  public:
    ...
    void write_temperature_records()
    {
        ...
        for (auto itr = temperature_records_out.begin();
             itr != temperature_records_out.end(); ++itr) {
    ...
    void read_thermal_zones()
    {
        ...
        for (auto itr = temperature_records_in.begin();
             itr != temperature_records_in.end(); ++itr) {
            file.open(itr->first);
            if (file.is_open()) {
